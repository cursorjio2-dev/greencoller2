...
class _LabourDetailsPageState extends State<LabourDetailsPage> {
  ...
  Widget _buildPhoneRow(String phone) {
    ...
    String displayText = _isUnlocked
        ? phone
        : WalletHelper.maskPhone(phone); // use your masking helper

    return InkWell(
      onTap: _isUnlocked ? () => _launchPhoneDialer(phone) : null,
      borderRadius: BorderRadius.circular(Constants.AppRadii.sm),
      child: Padding(
        padding: const EdgeInsets.symmetric(vertical: 10.0, horizontal: 4.0),
        child: Row(
          children: [
            ...
            if (!_isUnlocked)
              ...
              const Icon(Icons.chevron_right,
                  color: Constants.AppColors.inkSoft, size: 20),
          ],
        ),
      ),
    );
  ...
  Widget _buildStatItem(IconData icon, String value, String label) {
    return Column(
      children: [
        ...
        Text(
          translateText(label),
          style: Constants.AppTypography.micro
              .copyWith(color: Constants.AppColors.inkSoft),
        ),
      ],
    );
