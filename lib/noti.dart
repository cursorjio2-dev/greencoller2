...
import 'dart:convert';
import 'package:flutter/material.dart';
import 'package:greencollar/HomeScree.dart';      // <-- to use HomePage
import 'package:greencollar/constants.dart' as Constants;
import 'package:http/http.dart' as http;
import 'package:flutter_secure_storage/flutter_secure_storage.dart';
import 'package:translator/translator.dart';
...
class _NotificationsPageState extends State<NotificationsPage> {
  ...
  void _navigateToProject(BuildContext context, int? projectId) {
    if (projectId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No project associated with this notification.')),
      );
      return;
    }
    // Navigate to HomePage with the "Projects" tab (index 2) selected
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: isLoading
          ...
          itemBuilder: (context, index) {
            final notification = notifications[index];
            final projectId = notification['projectid'] as int?;
            return Container(
              margin: const EdgeInsets.symmetric(
                  vertical: 6, horizontal: 16),
              decoration: BoxDecoration(
              ...
              child: ListTile(
                ...
                trailing: notification['seen'] == '0'
                    ...
                    : const Icon(Icons.notifications_none,
                    color: Constants.AppColors.brand),
                onTap: () => _navigateToProject(context, projectId),
              ),
            );
          },
        ),
