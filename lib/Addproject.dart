...
//
// class SkillService {
//   static const String apiUrl = '${Constants.AppConstants.apiUrl}farmer/skill';
//
//   static Future<List<Skill>> fetchSkills() async {
//     try {
//       final response = await http.post(Uri.parse(apiUrl));
//       print("Dsadsadsadsadsadsadsadsdsd");
//       print(response.body);
//
//       if (response.statusCode == 200) {
//         final data = json.decode(response.body);
//         if (data['success'] == true) {
//           List<dynamic> skillData = data['data'];
//           return skillData.map((e) => Skill.fromJson(e)).toList();
//         } else {
...
//       }
//     } catch (e) {
//       throw Exception('Failed to load skills data: $e');
//     }
//   }
// }
//
...
//   final String? projectType; // Added projectType to constructor
//
//   // Constructor to accept the projectType argument
//   const AddProjectForm({Key? key, this.projectType}) : super(key: key);
//
//   @override
...
//           SnackBar(
//               content:
//                   Text(translateText('User ID not found in secure storage'))),
//         );
//         return;
//       }
//
...
//     print("Selected City : $selectedCity"); // Correct string interpolation
...
//       final String apiUrl =
//           '${Constants.AppConstants.apiUrl}farmer/registerProject';
//
...
//       );
//
//       // After receiving the response, re-enable the button
//       setState(() {
...
//           msg: translate('Project Added Successfully!',
//               'प्रोजेक्ट सफलतापूर्वक जोड़ा गया!'),
...
//                   'Failed to add project', 'प्रोजेक्ट जोड़ने में विफल'))),
//         );
//       }
//     } catch (e) {
//       // Handle errors
//       setState(() {
//         _isButtonDisabled =
//             false; // Ensure button is re-enabled if there's an error
//       });
//       print("Error: $e");
//     }
//   }
...
//
//         return AlertDialog(
//           backgroundColor: Constants.AppColors.card,
//           shape: RoundedRectangleBorder(
//             borderRadius: BorderRadius.circular(Constants.AppRadii.lg),
//           ),
//           title: Text(
//             translate('Add Milestone', 'माइलस्टोन जोड़ें'),
...
//                 decoration: _buildInputDecoration(
//                   translate('Milestone Amount', 'माइलस्टोन राशि'),
//                 ),
//                 style: Constants.AppTypography.body,
...
//           ),
//           title: Text(
//             translate('Edit Milestone', 'माइलस्टोन संपादित करें'),
//             style: Constants.AppTypography.h2,
//           ),
//           content: Column(
//             mainAxisSize: MainAxisSize.min,
...
//             descriptionController.text = _searchText;
//           });
//         },
//       );
//     } else {
...
//                           decoration: _buildInputDecoration(
//                             translate('Job/ Work/ Project Title', 'कार्य/कार्य/परियोजना का शीर्षक'),
//                           ),
//                           style: Constants.AppTypography.body,
...
//                       if (value == null || value.isEmpty) {
//                         return translate(
//                             'Please enter the number of required Workers.',
...
//                         child:
//                             Text(translate('6 months-1 year', '6 महीने-1 साल')),
//                       ),
//
...
//                               } else {
//                                 selectedSkills
//                                     .remove(skill.name); // Remove skill
//                               }
//
...
//                     onTap: () {
//                       setState(() {
//                         iscertVisible = !iscertVisible; // Toggle visibility
//                       });
//                     },
//                   ),
//
...
//                         return CheckboxListTile(
//                           title: Text(
//                             translateText(Qualification.name),
//                             style: Constants.AppTypography.body,
//                           ),
//                           activeColor: Constants.AppColors.brand,
//                           value: selectedcertifications
//                               .contains(Qualification.name),
//                           onChanged: (bool? selected) {
//                             setState(() {
//                               if (selected != null && selected) {
//                                 selectedcertifications.add(
//                                     Qualification.name); // Add qualification
//                               } else {
...
//                     decoration: _buildInputDecoration(
//                       translate('Enter Required Experience', 'आवश्यक अनुभव दर्ज करें'),
//                     ),
//                     style: Constants.AppTypography.body,
...
//                   DropdownButtonFormField<String>(
//                     decoration: _buildInputDecoration(
//                       translate('Payment Type', 'प्रोजेक्ट प्रकार'),
//                     ),
//                     style: Constants.AppTypography.body,
...
//                       children: [
//                         Text(
//                           translate('Milestones', 'माइलस्टोन'),
//                           style: Constants.AppTypography.h3,
//                         ),
//                         IconButton(
//                           icon: const Icon(Icons.add_circle, color: Constants.AppColors.brand),
...
//                       child: DataTable(
//                         headingRowColor: MaterialStateProperty.all(Constants.AppColors.surface2),
//                         columns: [
//                           DataColumn(
//                               label: Text(
//                             AppLocalizations.of(context)!.descriptionColumn,
//                             style: Constants.AppTypography.label,
//                           )),
//                           DataColumn(
//                               label: Text(
//                             AppLocalizations.of(context)!.amountColumn,
//                             style: Constants.AppTypography.label,
//                           )),
//                           DataColumn(
//                               label: Text(
...
//                               cells: [
//                                 DataCell(Text(
//                                   translateText(milestones[index]['description']),
//                                   style: Constants.AppTypography.body,
//                                 )),
//                                 DataCell(Text(
//                                   translateText(milestones[index]['amount'].toString()),
//                                   style: Constants.AppTypography.body,
//                                 )),
//                                 DataCell(Row(
//                                   children: [
...
//                           borderRadius: BorderRadius.circular(Constants.AppRadii.full),
//                           boxShadow: const [Constants.AppShadows.buttonFab],
//                         ),
...
class _AddProjectFormState extends State<AddProjectForm> {
  ...
  // ==================== FIX: Populate statesEn with API data ====================
  Future<void> _fetchStates() async {
    final response = await http
        .post(Uri.parse('${Constants.AppConstants.apiUrl}app/states'));
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        for (var state in states) {
          ...
          // Always store the raw title in statesEn (will be translated in UI)
          statesEn.add(stateTitle);
          statesHi.add(stateTitle); // keep for consistency
  ...
  // ==================== FIX: Always populate citiesEn ====================
  Future<void> _fetchOtherData(int stateCode) async {
    print(stateCode);
    final response = await http.post(
      Uri.parse('${Constants.AppConstants.apiUrl}app/cities'),
      body: {
        'stateid': stateCode.toString(),
      },
    );
    print(response.statusCode);
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        for (var city in cities) {
          String cityTitle = city['district_title'];
          // Always store raw title in citiesEn
          citiesEn.add(cityTitle);
          citiesHi.add(cityTitle);
        }
      });
    } else {
      throw Exception('Failed to load cities for state code $stateCode');
  ...
  String translateText(String text) {
    ...
    // ✅ Ensure _selectedLanguage is set
    ...
    // ✅ Fetch translation dynamically (background)
  ...
  void initState() {
    ...
    _loadSkillsData();
    // ========= FIX: Load language then fetch states =========
    loadLanguage().then((_) {
      _fetchStates();
    });
    _jobTypes = fetchJobTypes();
                  ...
                  children: [
                    FutureBuilder<List<JobType>>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          if (_selectedJobType == null && jobTypes.isNotEmpty) {
                            _selectedJobType = jobTypes[0].jobname;
                          }
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: experienceOptions.map((experience) {
                        return DropdownMenuItem<String>(
                          value: experience,
                          child: Text(translateText(experience)),
                        );
                      }).toList(),
                      onChanged: (value) {
                        setState(() {
                    ...
                    // ========== STATE DROPDOWN – uses statesEn and translateText ==========
                    DropdownButtonFormField<String>(
                      value: selectedState,
                      decoration: _buildInputDecoration(
                        AppLocalizations.of(context)!.state,
                    ...
                    const SizedBox(height: 16),
                    // ========== CITY DROPDOWN – uses citiesEn and translateText ==========
                    DropdownButtonFormField<String>(
                      value: selectedCity,
                      decoration: _buildInputDecoration(
                        AppLocalizations.of(context)!.city,
